...
скорость, ETA, текущий и общий размер. Живой probe показывает реальные форматы источника,
битрейт, кодек, FPS, контейнер и оценку размера; по умолчанию выбирается максимум.
URL задачи сохраняет несекретные query-параметры, а правая кнопка открывает команды
«Запустить», «Остановить», «Пауза», «Копировать URL» и «Удалить». Папка по умолчанию —
`Загрузки/JiveFetch`. Выпадающий список браузеров передаёт выбранный поддерживаемый
идентификатор в `yt-dlp --cookies-from-browser`; JiveFetch не копирует значения cookies.
...
- [Примечания к выпуску v0.3.0](docs/releases/v0.3.0.ru.md)
