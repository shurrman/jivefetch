...
- «Удалить» теперь открывает локализованный нативный вопрос «Да/Нет» об удалении связанных
  файлов с диска. «Нет» сохраняет файлы и удаляет только задачу; «Да» удаляет проверенный
  итоговый файл и partial workspace под управлением JiveFetch.
- Новые задачи получают внутренний artifact key и хранят resumable temp files в точном
  task-owned каталоге, не меняя пользовательское имя итогового файла.

### Безопасность

- Rust получает пути по task ID и expected revision; webview не может передать путь для
  удаления. Canonical checks отклоняют выход за output root и symlinks.
- Final file с другой task reference сохраняется. При unsafe path или ошибке очистки
  задача остаётся видимой, в том числе после restart.
- Перенесённые задачи до `0.4.2` сохраняют legacy layout: записанный final output можно
  удалить, но неизвестные legacy partials никогда не угадываются по имени.

### Метрики

- SQLite schema: с версии 6 до 7 с проверенной in-place migration nullable `artifact_key`.
- Обычный Rust suite: с 28 до 34 тестов плюс opt-in real-engine loopback smoke.
- Remove: вместо 1 немедленного queue-only действия доступны 2 явных исхода keep/delete
  files на каждом из 3 языков приложения.

## 0.4.1 - 2026-09-06

### Исправлено

- При Resume карточка задачи больше не сбрасывается на 0%, пока `yt-dlp` уже подключается
  к совместимым partial-файлам. Reservation сохраняет durable checkpoint прогресса, а UI
  до новых данных движка показывает локализованный этап «Подготовка к продолжению».
- Для продолженного компонента первая наблюдаемая позиция bytes становится локальным
  baseline попытки, а к checkpoint добавляется только дальнейший прирост. Это исключает
  и видимый откат, и двойной учёт ранее загруженных bytes.

### Метрики

- Обычный Rust suite: с 26 до 28 тестов, включая сохранение checkpoint через durable
  Pause/Resume и агрегацию нескольких компонентов после продолжения.
- Состояние Resume: вместо сброса 3 progress-полей сохраняются 3 checkpoint-поля;
  локализованных этапов: с 4 до 5 в каждом языке приложения.
- SQLite schema остаётся версии 6; migration существующей очереди не требуется.
