...
отдельного компонента сам по себе не означает complete. UI запрашивает
...
Начиная с `0.4.1`, reservation продолженной попытки сохраняет последний подтверждённый
progress, загруженные bytes и оценку общего размера. До первого нового engine event UI
показывает локализованный этап «Подготовка к продолжению». Первая наблюдаемая позиция
каждого компонента становится baseline, а к checkpoint добавляется только дальнейший
прирост: повторно используемые partial bytes не исчезают и не считаются дважды.
